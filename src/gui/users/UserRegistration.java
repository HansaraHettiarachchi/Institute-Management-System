...
public class UserRegistration extends javax.swing.JDialog {
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        if (validation()) {
            ...
            if (cando) {
                String query = "INSERT INTO `user` (`email`,`ps`,`nic`, `fname`, `lname`, `registeredDate`, `dob`, `mobile`, `status_id`, `paymentType_id`, `gender_id`, `userType_id`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?); ";

                Date d = new Date();
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                SimpleDateFormat sdf1 = new SimpleDateFormat("yyyy-MM-dd");

                Object val[] = new Object[]{email, ps, nic, fname, lname, sdf.format(d), sdf1.format(date), mobile, statusMap.get(status), 1, genderMap.get(gen), userTMap.get(uType)};
                long insert_id = 0;
