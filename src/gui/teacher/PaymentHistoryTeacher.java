 ...
 * @author nuwan
 */
public class PaymentHistoryTeacher extends javax.swing.JDialog {

    private String tnic;

    public PaymentHistoryTeacher(java.awt.Frame parent, boolean modal, String tnic) {
        super(parent, modal);
        initComponents();
        this.tnic = tnic;

        loadTeacherDetails(tnic);
        populateYearAndMonthFilters();
        loadPaymentHistory(this.tnic);
    ...
    private void loadTeacherDetails(String Tnic) {
        try {
            String query = "SELECT * FROM teachers WHERE nic = '" + Tnic + "';";
    ...
    private void populateYearAndMonthFilters() {
        try {
            String query = "SELECT DISTINCT YEAR(DAT) AS year FROM teapayments;";
            ResultSet rs = MySql.select(query);
    ...
    private void loadPaymentHistory(String TeacherNIC) {
        try {
            String yearFilter = (String) jComboBox3.getSelectedItem();
            String monthFilter = (String) jComboBox4.getSelectedItem();

            String query = "SELECT tp.id, tp.DAT, tp.price, tp.paymentType_id, tp.desc, pt.name AS payment_method, t.name, t.nic\n"
                    + "FROM teapayments tp\n"
                    + "INNER JOIN teachers t ON tp.teachers_id = t.id\n"
                    + "INNER JOIN paymenttype pt ON tp.paymentType_id = pt.id\n"
                    + "WHERE t.nic = '" + TeacherNIC + "' ";

            if (yearFilter != null && !yearFilter.equals("All Years")) {
                query += "AND YEAR(tp.DAT) = '" + yearFilter + "' ";
            }

            if (monthFilter != null && !monthFilter.equals("All Months")) {
                int monthNumber = getMonthNumber(monthFilter);
                if (monthNumber > 0) {
                    query += "AND MONTH(tp.DAT) = " + monthNumber + " ";
                }
            }

            query += "ORDER BY tp.DAT DESC";
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(PaymentHistoryTeacher.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
